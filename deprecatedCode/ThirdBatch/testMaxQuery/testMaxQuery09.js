	...
	if (selectedOpt == '#'){ //'#' Mark as program terminate
		...
		if (selectedOpt && optionMarker >= 0 && optionMarker < optionalKeywords.length){ //Check if user input is a number and the number is in available range
			...
			if (replicateCheck && !timeKeyCheck){ //If found any relicated keyword then ask if user want to replace the value && the keyword is not a time keyword
				...
				var optionalKeyInputFrom = 10000101000000;
				var optionalKeyInputTo = 30001231000000;
				...
				var optionalKeyInput = 'empty';
				if (selectedOpt == 1){
					optionalKeyInput = 'Treatment Plan or Protocol'; //same
				}
				else if (selectedOpt == 2){
					optionalKeyInput = 'LABORATORY REPORT.TOTAL'; //same
				}
				else if (selectedOpt == 3){
					optionalKeyInput = 'Gastroenterology';
				}
				else if (selectedOpt == 7){
					optionalKeyInput = 'Hospital outpatient gastroenterology clinic';
				}
				else if (selectedOpt == 9){
					optionalKeyInput = 'Restricted'; //same
				}
