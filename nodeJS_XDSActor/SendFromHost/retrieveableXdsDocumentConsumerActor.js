...
const selectedMethod = process.argv.slice(2);
var selectedMethodArray = [];
var HOST = {};
var PORT = {};
var client = {};
var chunkSize = 1024;
...
const queryTypeList = ['', 'FindDocuments', 'FindSubmissionSets', 'FindFolders',
...
const timeKeyList = ['$XDSDocumentEntryCreationTimeFrom', '$XDSDocumentEntryCreationTimeTo',
...
function Main() { //Standard program which provide command line UI for input
	...
	console.log('==================================');
	console.log('Please select XDS Consumer Actor Role');
	console.log('1) Document Query');
	console.log('2) Document Retrieve');
	console.log('#) Quit');
...
function createXML(sHOST, sPORT) { //Assort keywords into ITI-18 XML format
	...
	//re-check expected return type
	if (inputKeywords[0].includes('Find')) {
		queryXML['query:AdhocQueryRequest']['query:ResponseOption'][0]['$']['returnType'] = 'ObjectRef';
	}
	else if (inputKeywords[0].includes('Get')) {
		queryXML['query:AdhocQueryRequest']['query:ResponseOption'][0]['$']['returnType'] = 'LeafClass';
	}
	else {
		console.log('Response Option unidentified, default as Leafclass');
	}
	console.log('||||||||||||||||||||||||||||||||||||||||||||||||||||');
	console.log(util.inspect(queryXML['query:AdhocQueryRequest']['query:ResponseOption'], { depth: null, colors: true }));
	console.log('====================================================');
	console.log(util.inspect(queryXML['query:AdhocQueryRequest']['rim:AdhocQuery'], { depth: null, colors: true }));
	console.log('||||||||||||||||||||||||||||||||||||||||||||||||||||');
	for (i = 1; i < inputKeywords.length; i++) {
		var rimSlot = {
			"$": {
		...
		queryXML['query:AdhocQueryRequest']['rim:AdhocQuery'][0]['rim:Slot'].push(rimSlot);
	}
	console.log(util.inspect(queryXML['query:AdhocQueryRequest']['query:ResponseOption'], { depth: null, colors: true }));
	console.log('====================================================');
	console.log(util.inspect(queryXML['query:AdhocQueryRequest']['rim:AdhocQuery'], { depth: null, colors: true }));
...
const forExpDocSearchKeywords = {
	'01': {
		requiredKeywords: [
			['XDSDocumentEntryPatientId', 'IHEBLUE-2736^^^&1.3.6.1.4.1.21367.13.20.3000&ISO'],
		...
		getDocKeywords: [
			...
			['XDSDocumentEntryUniqueId', 'experimentUniqueId01-780d23a4dd7b5514289a32b38bc6c167']
		]
	},
	'02': {
		requiredKeywords: [
			['XDSDocumentEntryPatientId', 'IHEBLUE-2736^^^&1.3.6.1.4.1.27829.13.20.3000&ISO'],
			['XDSDocumentEntryStatus', 'urn:oasis:names:tc:ebxml-regrep:StatusType:Approved']
		],
		optionalKeywords: [
			['XDSDocumentEntryClassCode', 'Treatment Plan or Protocol'],
			['XDSDocumentEntryTypeCode', 'LABORATORY REPORT.TOTAL'],
			['XDSDocumentEntryPracticeSettingCode', 'Neurology'],
			['XDSDocumentEntryCreationTime', 20070101],
			['XDSDocumentEntryServiceStartTime', 200701011800],
			['XDSDocumentEntryServiceStopTime', 200701011900],
			['XDSDocumentEntryHealthcareFacilityTypeCode', 'Hospital-trauma center'],
			['XDSDocumentEntryConfidentialityCode', 'Restricted'],
			['XDSDocumentEntryAuthorPerson', '^Tymoteusz^Mccabe^^^'],
			['XDSDocumentEntryAuthorInstitution', 'Kentucky Hospital'],
			['XDSDocumentEntryAuthorRole', 'Attending'],
			['XDSDocumentEntryAuthorSpecialty', 'Neurology'],
			['XDSDocumentEntryFormatCode', 'urn:ihe:iti:bppc:2007'],
			['XDSDocumentEntryType', 'urn:uuid:7edca82f-054d-47f2-a032-9b2a5b5186c1']
		],
		getDocKeywords: [
			['XDSDocumentEntryEntryUUID', 'Document02'],
			['XDSDocumentEntryUniqueId', 'experimentUniqueId02-b27d9f1d8c2a2a3d11ee0269e4f9bb72']
		]
	},
	'03': {
		requiredKeywords: [
			['XDSDocumentEntryPatientId', 'IHEBLUE-2736^^^&1.3.6.1.4.1.56658.13.20.3000&ISO'],
			['XDSDocumentEntryStatus', 'urn:oasis:names:tc:ebxml-regrep:StatusType:Approved']
		],
		optionalKeywords: [
			['XDSDocumentEntryClassCode', 'Treatment Plan or Protocol'],
			['XDSDocumentEntryTypeCode', 'LABORATORY REPORT.TOTAL'],
			['XDSDocumentEntryPracticeSettingCode', 'Rehabilitation'],
			['XDSDocumentEntryCreationTime', 20070215],
			['XDSDocumentEntryServiceStartTime', 200702151000],
			['XDSDocumentEntryServiceStopTime', 200702151100],
			['XDSDocumentEntryHealthcareFacilityTypeCode', 'Hospital-rehabilitation'],
			['XDSDocumentEntryConfidentialityCode', 'Restricted'],
			['XDSDocumentEntryAuthorPerson', '^Sharna^Hood^^^'],
			['XDSDocumentEntryAuthorInstitution', 'Springdale Clinic'],
			['XDSDocumentEntryAuthorRole', 'Attending'],
			['XDSDocumentEntryAuthorSpecialty', 'Physiotherapy'],
			['XDSDocumentEntryFormatCode', 'urn:ihe:iti:bppc:2007'],
			['XDSDocumentEntryType', 'urn:uuid:7edca82f-054d-47f2-a032-9b2a5b5186c1']
		],
		getDocKeywords: [
			['XDSDocumentEntryEntryUUID', 'Document03'],
			['XDSDocumentEntryUniqueId', 'experimentUniqueId03-19503cce414173e8a27060de24a5ec0e']
		]
	},
	'04': {
		requiredKeywords: [
			['XDSDocumentEntryPatientId', 'IHEBLUE-2736^^^&1.3.6.1.4.1.50877.13.20.3000&ISO'],
			['XDSDocumentEntryStatus', 'urn:oasis:names:tc:ebxml-regrep:StatusType:Approved']
		],
		optionalKeywords: [
			['XDSDocumentEntryClassCode', 'Treatment Plan or Protocol'],
			['XDSDocumentEntryTypeCode', 'LABORATORY REPORT.TOTAL'],
			['XDSDocumentEntryPracticeSettingCode', 'Urological oncology'],
			['XDSDocumentEntryCreationTime', 20070225],
			['XDSDocumentEntryServiceStartTime', 200702250800],
			['XDSDocumentEntryServiceStopTime', 200702250900],
			['XDSDocumentEntryHealthcareFacilityTypeCode', 'Hospital-long term care'],
			['XDSDocumentEntryConfidentialityCode', 'Restricted'],
			['XDSDocumentEntryAuthorPerson', '^Bear^Ryan^^^'],
			['XDSDocumentEntryAuthorInstitution', 'Westbury Hospital'],
			['XDSDocumentEntryAuthorRole', 'Attending'],
			['XDSDocumentEntryAuthorSpecialty', 'Urology'],
			['XDSDocumentEntryFormatCode', 'urn:ihe:iti:bppc:2007'],
			['XDSDocumentEntryType', 'urn:uuid:7edca82f-054d-47f2-a032-9b2a5b5186c1']
		],
		getDocKeywords: [
			['XDSDocumentEntryEntryUUID', 'Document04'],
			['XDSDocumentEntryUniqueId', 'experimentUniqueId04-6206fea3cbcfe50c1fb0eb3da2e1b63b']
		]
	},
	'05': {
		requiredKeywords: [
			['XDSDocumentEntryPatientId', 'IHEBLUE-2736^^^&1.3.6.1.4.1.20232.13.20.3000&ISO'],
			['XDSDocumentEntryStatus', 'urn:oasis:names:tc:ebxml-regrep:StatusType:Approved']
		],
		optionalKeywords: [
			['XDSDocumentEntryClassCode', 'Treatment Plan or Protocol'],
			['XDSDocumentEntryTypeCode', 'LABORATORY REPORT.TOTAL'],
			['XDSDocumentEntryPracticeSettingCode', 'Nephrology'],
			['XDSDocumentEntryCreationTime', 20070302],
			['XDSDocumentEntryServiceStartTime', 200703021300],
			['XDSDocumentEntryServiceStopTime', 200703021530],
			['XDSDocumentEntryHealthcareFacilityTypeCode', 'Dialysis unit--hospital'],
			['XDSDocumentEntryConfidentialityCode', 'Restricted'],
			['XDSDocumentEntryAuthorPerson', '^Katya^Cairns^^^'],
			['XDSDocumentEntryAuthorInstitution', 'Winter Park Hospital'],
			['XDSDocumentEntryAuthorRole', 'Attending'],
			['XDSDocumentEntryAuthorSpecialty', 'Nephrology'],
			['XDSDocumentEntryFormatCode', 'urn:ihe:iti:bppc:2007'],
			['XDSDocumentEntryType', 'urn:uuid:7edca82f-054d-47f2-a032-9b2a5b5186c1']
		],
		getDocKeywords: [
			['XDSDocumentEntryEntryUUID', 'Document05'],
			['XDSDocumentEntryUniqueId', 'experimentUniqueId05-4e48537b89aaa1179f07f2abd9068cad']
		]
	},
	'06': {
		requiredKeywords: [
			['XDSDocumentEntryPatientId', 'IHEBLUE-2736^^^&1.3.6.1.4.1.90654.13.20.3000&ISO'],
			['XDSDocumentEntryStatus', 'urn:oasis:names:tc:ebxml-regrep:StatusType:Approved']
		],
		optionalKeywords: [
			['XDSDocumentEntryClassCode', 'Treatment Plan or Protocol'],
			['XDSDocumentEntryTypeCode', 'LABORATORY REPORT.TOTAL'],
			['XDSDocumentEntryPracticeSettingCode', 'Cardiology'],
			['XDSDocumentEntryCreationTime', 20070415],
			['XDSDocumentEntryServiceStartTime', 200704151000],
			['XDSDocumentEntryServiceStopTime', 200704151030],
			['XDSDocumentEntryHealthcareFacilityTypeCode', 'Hospital-cardiology unit'],
			['XDSDocumentEntryConfidentialityCode', 'Restricted'],
			['XDSDocumentEntryAuthorPerson', '^Imani^Heath^^^'],
			['XDSDocumentEntryAuthorInstitution', 'City Hospital'],
			['XDSDocumentEntryAuthorRole', 'Attending'],
			['XDSDocumentEntryAuthorSpecialty', 'Cardiology'],
			['XDSDocumentEntryFormatCode', 'urn:ihe:iti:bppc:2007'],
			['XDSDocumentEntryType', 'urn:uuid:7edca82f-054d-47f2-a032-9b2a5b5186c1']
		],
		getDocKeywords: [
			['XDSDocumentEntryEntryUUID', 'Document06'],
			['XDSDocumentEntryUniqueId', 'experimentUniqueId06-efc5a19d82c0b1d8f64791370dbe84a5']
		]
	},
	'07': {
		requiredKeywords: [
			['XDSDocumentEntryPatientId', 'IHEBLUE-2736^^^&1.3.6.1.4.1.41480.13.20.3000&ISO'],
			['XDSDocumentEntryStatus', 'urn:oasis:names:tc:ebxml-regrep:StatusType:Approved']
		],
		optionalKeywords: [
			['XDSDocumentEntryClassCode', 'Treatment Plan or Protocol'],
			['XDSDocumentEntryTypeCode', 'LABORATORY REPORT.TOTAL'],
			['XDSDocumentEntryPracticeSettingCode', 'General practice'],
			['XDSDocumentEntryCreationTime', 20070403],
			['XDSDocumentEntryServiceStartTime', 200704030900],
			['XDSDocumentEntryServiceStopTime', 200704031000],
			['XDSDocumentEntryHealthcareFacilityTypeCode', 'Hospital-community'],
			['XDSDocumentEntryConfidentialityCode', 'Restricted'],
			['XDSDocumentEntryAuthorPerson', '^Mujtaba^Palacios^^^'],
			['XDSDocumentEntryAuthorInstitution', 'Nevada Hospital'],
			['XDSDocumentEntryAuthorRole', 'Attending'],
			['XDSDocumentEntryAuthorSpecialty', 'Physiology'],
			['XDSDocumentEntryFormatCode', 'urn:ihe:iti:bppc:2007'],
			['XDSDocumentEntryType', 'urn:uuid:7edca82f-054d-47f2-a032-9b2a5b5186c1']
		],
		getDocKeywords: [
			['XDSDocumentEntryEntryUUID', 'Document07'],
			['XDSDocumentEntryUniqueId', 'experimentUniqueId07-403ba00121f30929ace67d9bdb45d21c']
		]
	},
	'08': {
		requiredKeywords: [
			['XDSDocumentEntryPatientId', 'IHEBLUE-2736^^^&1.3.6.1.4.1.88518.13.20.3000&ISO'],
			['XDSDocumentEntryStatus', 'urn:oasis:names:tc:ebxml-regrep:StatusType:Approved']
		],
		optionalKeywords: [
			['XDSDocumentEntryClassCode', 'Treatment Plan or Protocol'],
			['XDSDocumentEntryTypeCode', 'LABORATORY REPORT.TOTAL'],
			['XDSDocumentEntryPracticeSettingCode', 'Clinical physiology'],
			['XDSDocumentEntryCreationTime', 20070311],
			['XDSDocumentEntryServiceStartTime', 200703111000],
			['XDSDocumentEntryServiceStopTime', 200703111030],
			['XDSDocumentEntryHealthcareFacilityTypeCode', 'Local community health center'],
			['XDSDocumentEntryConfidentialityCode', 'Restricted'],
			['XDSDocumentEntryAuthorPerson', '^Mahima^Ho^^^'],
			['XDSDocumentEntryAuthorInstitution', 'Bungalow Clinic'],
			['XDSDocumentEntryAuthorRole', 'Attending'],
			['XDSDocumentEntryAuthorSpecialty', 'Physiology'],
			['XDSDocumentEntryFormatCode', 'urn:ihe:iti:bppc:2007'],
			['XDSDocumentEntryType', 'urn:uuid:7edca82f-054d-47f2-a032-9b2a5b5186c1']
		...
		getDocKeywords: [
			['XDSDocumentEntryEntryUUID', 'Document08'],
			['XDSDocumentEntryUniqueId', 'experimentUniqueId08-14dd9ef9a2233f1c90571c4d131512c3']
		]
	},
	'09': {
		requiredKeywords: [
			['XDSDocumentEntryPatientId', 'IHEBLUE-2736^^^&1.3.6.1.4.1.11223.13.20.3000&ISO'],
			['XDSDocumentEntryStatus', 'urn:oasis:names:tc:ebxml-regrep:StatusType:Approved']
		],
		optionalKeywords: [
			['XDSDocumentEntryClassCode', 'Treatment Plan or Protocol'],
			...
			['XDSDocumentEntryCreationTime', 20070421],
			['XDSDocumentEntryServiceStartTime', 200704210930],
			['XDSDocumentEntryServiceStopTime', 200704211030],
			['XDSDocumentEntryHealthcareFacilityTypeCode', 'Orthopedic clinic'],
			['XDSDocumentEntryConfidentialityCode', 'Restricted'],
			['XDSDocumentEntryAuthorPerson', '^Zayne^Clements^^^'],
			['XDSDocumentEntryAuthorInstitution', 'Orthopedic Clinic'],
			['XDSDocumentEntryAuthorRole', 'Attending'],
			['XDSDocumentEntryAuthorSpecialty', 'Orthopedics'],
			['XDSDocumentEntryFormatCode', 'urn:ihe:iti:bppc:2007'],
			['XDSDocumentEntryType', 'urn:uuid:7edca82f-054d-47f2-a032-9b2a5b5186c1']
		],
		getDocKeywords: [
			['XDSDocumentEntryEntryUUID', 'Document09'],
			['XDSDocumentEntryUniqueId', 'experimentUniqueId09-7849d5e831839e89d2d4c5d9174c6a5e']
		]
	},
	'10': {
		requiredKeywords: [
			['XDSDocumentEntryPatientId', 'IHEBLUE-2736^^^&1.3.6.1.4.1.00234.13.20.3000&ISO'],
			['XDSDocumentEntryStatus', 'urn:oasis:names:tc:ebxml-regrep:StatusType:Approved']
		],
		optionalKeywords: [
			['XDSDocumentEntryClassCode', 'Treatment Plan or Protocol'],
		...
		getDocKeywords: [
			...
			['XDSDocumentEntryUniqueId', 'experimentUniqueId10-7cab01cc7688efc8c7b400fbb6ee6ec1']
		]
	},
	'11': {
		requiredKeywords: [
			['XDSDocumentEntryPatientId', 'IHEBLUE-2736^^^&1.3.6.1.4.1.00250.13.20.3000&ISO'],
			['XDSDocumentEntryStatus', 'urn:oasis:names:tc:ebxml-regrep:StatusType:Approved']
		],
		optionalKeywords: [
			['XDSDocumentEntryClassCode', 'Treatment Plan or Protocol'],
		...
		getDocKeywords: [
			['XDSDocumentEntryEntryUUID', 'Document11'],
			['XDSDocumentEntryUniqueId', 'experimentUniqueId11-9af5f3badb2594250bc3378f2ff7e6f1']
		]
	}
};
...
function getRepositoryAct() {
	console.log('Retrieve Document Set [ITI-43]...');
	...
	fs.readFile(savedMetaAtt, function (err, buf) {
		...
		parseString(savedXmlMessage, function (err, result) {
			...
			RegisterDocumentSetB(); //Should proceed after got ITI-43 response
		});
	});
}
...
var repoSendNextChunk = function () {
	if (m < repoXmlChunks.length) {
		repoClient.write(repoXmlChunks[m], function () {
			m++;
		});
	}
};

function registerToLocalRepo(retrieveResponseXML) {
	var localRepoClient = new net.Socket();
	var localRepoHost = 'localhost';
	var localRepoPort = '9876';
	localRepoClient.connect(localRepoPort, localRepoHost, function () {
		console.log('CONNECTED TO: ' + registHost + ':' + registPORT);
		//Not full function yet here
		localRepoClient.write('trigger local ITI-42');
		console.log('Document Register request sent...');
	});

	localRepoClient.on('data', function (data) {
		var dataGot = data.toString();
		if (dataGot == "Register Success") {
			console.log('==============================');
...
function quickExecute() {
	roundCountCheck++;
	multipleServerStatus = false; //Set this to TRUE when want to perform document query on all nodes at once
	inputKeywords = [];
	console.log('||||||||||||||||||||||||||||||||||||||||||||||||||||');
	console.log(inputKeywords);
	console.log('||||||||||||||||||||||||||||||||||||||||||||||||||||');
	var pseudoSingleSetValue = false;
	queryXML = JSON.parse(JSON.stringify(moldQueryXML));
	if (selectedQueryAndExperiment == "FindMinSingle") {
		pseudoSingle = pseudoSingleSetValue;
	...
	else if (selectedQueryAndExperiment == "FindMinAll") {
		queryType = "FindDocuments";
		inputKeywords.push(queryType);
	...
	else if (selectedQueryAndExperiment == "FindMaxSingle") {
		pseudoSingle = pseudoSingleSetValue;
		queryType = "FindDocuments";
		inputKeywords.push(queryType);
		//add requierd keywords
	...
	else if (selectedQueryAndExperiment == "FindMaxAll") {
		queryType = "FindDocuments";
		inputKeywords.push(queryType);
		//add requierd keywords
		for (var i = 0; i < forExpDocSearchKeywords[selectedDoc].requiredKeywords.length; i++) {
	...
	else if (selectedQueryAndExwddddddwperiment == "GetDocSingle") {
		pseudoSingle = pseudoSingleSetValue;
	...
	else if (selectedQueryAndExperiment == "GetDocAll") {
		queryType = "GetDocuments";
		inputKeywords.push(queryType);
		// add GetDoc keywords
		for (var i = 0; i < forExpDocSearchKeywords[selectedDoc].getDocKeywords.length; i++) {
...
var selectedQueryAndExperiment = null;
const sampleNumber = ['00', '01', '02', '03', '04', '05', '06', '07', '08', '09', '10'];
if (selectedMethod.length > 1) {
	//node retrieveableXdsDocumentConsumerActor.js 01 FindMinSingle 10 10
	selectedMethodArray = selectedMethod;
	selectedDoc = selectedMethodArray[0];
	selectedQueryAndExperiment = selectedMethodArray[1];
	if (selectedMethod[2]) {
		repeatTimes = selectedMethod[2];
	}
	if (selectedMethod[3]) {
		nextRound = selectedMethod[3];
	}
	quickExecute();
}
...
if (repeatTimes) {
	var expServer = net.createServer((socket) => {
		...
		socket.on('data', function (data) {
			...
			if (dataGot == 'Ready') {
				repeatTimes--;
				console.log('Repeat remaining: ' + repeatTimes);
				console.log('Total round: ' + roundCountCheck);
				if (repeatTimes > 0) {
					console.log('Continue repeat...');
					selectedDoc = sampleNumber[11 - repeatTimes];
					console.log('Proceed on document number: ' + selectedDoc);
					quickExecute();
				}
				else {
