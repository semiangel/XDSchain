...
var netServer = null;
var netSocket = null;
var messageArray = []; // Array to store small packets
var messageChunks = 2;
...
async function documentQuery (inputAttributes) {
	...
	function sendBackQueryResult (responseXML){
		...
		if (netServer && netSocket) {
			...
			var xmlMessage = responseXML.toString();
	        //var regex = /\r?\n|\r/g;
			...
			var sendNextChunk = function() {
				if (i < xmlChunks.length) {
					netSocket.write(xmlChunks[i], function() {
						i++;
						sendNextChunk();
					});
				} else {
	...
	sock.on('data', function(data) {
		...
		if (receiveCounter == 0){
			...
			if(receiveCounter == messageChunks){
				...
				// reset the messageArray and receiveCounter
				messageArray = [];
				receiveCounter = 0;
			}
		}
