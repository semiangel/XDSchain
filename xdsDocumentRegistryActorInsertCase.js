...
function documentQuery (inputAttributes) {
	...
	async function checkLastID (sK) {
	  ...
	  myContract.methods.checkLastID().call({
	    ...
	    if (err) {
	      ...
	      console.log('------------------------------------------');
	      invokeContract(sK, res);
	    }
	  });  
	}

	//Invoke each contract for keyword search
	async function invokeContract(sK, maxDoc){
	  //web3.eth.defaultAccount = web3.eth.personal.getAccounts().then(console.log);
	  ...
	  for (Docid = 1; Docid <= maxDoc; Docid++){
	    ...
	    myContract.methods.retreiveSearch(Docid).call({
	      ...
	      if (err) {
	        ...
	        matchMaker(XDSattributes, sK, traceDocid);
	      }
	    });
	  }  
	}
	...
	function matchMaker (searchXDSAtt, sK, Docid){
		var responseOption = sK[sK.length - 1]; //The last position of sK variable contain responseOption value
		if (responseOption == 'LeafClass') {
			matchForLeafClass(searchXDSAtt, sK, Docid);
		}
		else if (responseOption == 'ObjectRef') {
			matchForObjectRef(searchXDSAtt, sK, Docid);
		}
	}

	//Both match maker program ignore the last cell in array
	function matchForLeafClass (searchXDSAtt, sK, Docid) {
	...
	function matchForObjectRef (searchXDSAtt, sK, Docid, myCallback) {
		//searchXDSAtt = XDS Object called from smartcontract
		//sK = search keyword received from ITI-18
		console.log('----------\nSmartcontract called...');
		var matchedCount = 0;
...
//**************************************************************************************EditingHere
//For ObjectRef the search continue search until the last and get entryUUID of all matched result to return
		if (matchedCount == sK.length - 1){
			console.log('All matched, successfully... \nReturn document as search result:\n========================================');
			console.log(searchXDSAtt.DocumentEntry);
			console.log('========================================');
			objectRefList.push([searchXDSAtt['DocumentEntry']['entryUUID']);
			//otherFunction to get all matching ObjectRef
			//Get directly from matcher contract just entryUUID
			//save it to array and pass it to responseAsObjectRef
			//fullContract(Docid, sK);
		}
		else {
			console.log('Unmatched...');
	...
	async function fullContract (Docid, sK){
	  //web3.eth.defaultAccount = web3.eth.personal.getAccounts().then(console.log);
	  ...
	  console.log('Returning document: ' + Docid);
		...
		from: deployerAccount
			...
			if (err) {
				...
				var XDSattributes = JSON.parse(res);
				responseToUser(XDSattributes, sK);
			}
		}); 
	}
	...
	function responseToUser (rXDSattribute, sK) {
		if (responseOption == 'LeafClass') {
			responseAsLeafClass(rXDSattribute, sK);
		}
		else if (responseOption == 'ObjectRef') {
			responseAsObjectRef(rXDSattribute, sK);
		}
	}

	function responseAsLeafClass (rXDSattribute, sK) {
		//Define variable for shorter object accessing
		var sEnvelope = rXDSattribute['soapenv:Envelope'];
		//inside Enveope
		var s$ = sEnvelope['$'];
	...
	function responseAsObjectRef (rXDSattribute, sK) {
	...
	function specifyRequestType (requestedJSON) {
	  ...
	  checkLastID(searchKeyword);
	}
	specifyRequestType(inputAttributes);
