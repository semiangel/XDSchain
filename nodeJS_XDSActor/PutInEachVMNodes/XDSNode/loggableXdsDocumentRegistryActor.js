...
//The IP value, this implementation assigned a static value in each machine to avoid mistake from programming
//var HOST = require('./ip.js'); //cancel using ip value from ip.js, now use os to get local ip instead
var PORT = require('./port.js'); //The port still use a simple static assign method

var netServer = null;
var netSocket = null;

var clientIP = {}; //Object to store message packet component bound to each client
var chunkSize = 1024;
...
var experimentMark = null;

if (argumentInput) {
	if (argumentInput.includes('-exp')) {
		experimentMark = true;
	}
}

//===========================================================================================================
//Experiment available connection check and ready telling
//===========================================================================================================
const testIp = '192.168.1.122';
const testPorts = [5656, 5757, 5858];
const testMessage = 'Ready';

async function checkPort(port) {
  return new Promise((resolve) => {
    const client = new net.Socket();
    
    client.setTimeout(3000); // Set a timeout of 3 seconds

    client.connect(port, testIp, () => {
      console.log(`Connected to ${testIp}:${port}`);
...
async function readyForExperiment() {
	console.log('Ready for experiment checking...');
	for (const port of testPorts) {
		const isConnected = await checkPort(port);
		if (isConnected) {
			console.log(`Ready sent to ${testIp}:${port}`);
...
//Get some basic info. from local machine
//Get local IP for log
...
var HOST = thisMachineIP; //Assign local ip for tecp connection

//RegisterDocumentSet-b=============================================================================================
//-----------------------------------------------
//Declare main object to store all META-data attributes essential for search operation
function registerDocumentSetb(inputAttributes, fromIP) {
	var prepXDSAtt = {
		DocumentEntry: {
			author: [{
				authorPerson: 'N/A',
	...
	async function invokeContract(XDSMETADataAttributes, rawXDSAttr) {
		...
		myContract.methods.store(Docid, inputSearch, inputFull).send({
			...
			console.log(receipt);
			if (receipt) {
				logginContract('registerSuccess', fromIP, null, null, XDSMETADataAttributes.DocumentEntry.uniqueId);
			}
			console.log('====================\nTransaction success...');
			hrend = process.hrtime(hrstart);
			var totalMillisec = hrend[0] * 1000 + (hrend[1] / 1000000);
			var testResult = '\nExecution time (hr): ' + totalMillisec;
			...
			console.log('====================');
			//netSocket.write("Published"); Can't remember what this suppose to do?
...
//DocumentQuery=====================================================================================================
async function documentQuery(inputAttributes, fromIP) {
	var requestUUID = {
	...
	//Send number of chunk to let the receiver know - Specific for Query
	var prepareQueryResultChunk = function () {
		var chunkCount = clientIP[netSocket.remoteAddress].xmlChunks.length + 1; //+1 also count the chunkCount value
		console.log("***************************");
		console.log("Chunk pieces: " + chunkCount);
		console.log("***************************");
		if (chunkCount) {
			console.log("sending chunkCount");
			netSocket.write(chunkCount.toString()); //.write send only string
		}
		hrstart = process.hrtime();
		//console.log('Sent: \n' + clientIP[netSocket.remoteAddress].xmlMessage + '\n');     
	}

	// Send packets one by one - Specific for Query
	clientIP[netSocket.remoteAddress].i = 0;
	// Move this function outside scope of documentQuery to allow interaction with socket

	function sendBackQueryResult(responseJSON) {
		//This function get response from corresponding case and send back to XDSConsumer
		if (netServer && netSocket) {
			console.log('Responsing query result: ');
			var responseXML = builder.buildObject(responseJSON);
			clientIP[netSocket.remoteAddress].xmlMessage = responseXML.toString();

			// Split XML message into smaller packets
			clientIP[netSocket.remoteAddress].xmlChunks = [];
			var chunk = "";
			for (var i = 0; i < clientIP[netSocket.remoteAddress].xmlMessage.length; i++) {
				chunk += clientIP[netSocket.remoteAddress].xmlMessage[i];
				if (chunk.length === chunkSize || i === clientIP[netSocket.remoteAddress].xmlMessage.length - 1) {
					clientIP[netSocket.remoteAddress].xmlChunks.push(chunk);
					chunk = "";
				}
			}
			prepareQueryResultChunk(); //Prepare and enter query result packets sending loop
			console.log('-----------------------');
			//console.log(responseXML);
			console.log('Query Result Sent!');
		}
		console.log('========================================\nDone!');
		hrend = process.hrtime(hrstart);
		var totalMillisec = hrend[0] * 1000 + (hrend[1] / 1000000);
		console.info('Execution time (hr): %d', totalMillisec);
		console.log('========================================');
	}
	...
	function responseAsLeafClass(rXDSattribute) {
		...
		var sEnvelope = rXDSattribute['soapenv:Envelope'];
		//inside Enveope
		var s$ = sEnvelope['$']; // variable not yet implemented
		var sBody = sEnvelope['soapenv:Body'][0];
		var sHeader = sEnvelope['soapenv:Header'][0];
		//inside Envelope>Header
		var wsaTo = sHeader['wsa:To']; // variable not yet implemented
		var wsaMessageID = sHeader['wsaMessageID']; // variable not yet implemented
		var wsaAction = sHeader['wsaAction']; // variable not yet implemented
		//inside Envelope>Body
		var lcmSubmitObjectsRequest = sBody['lcm:SubmitObjectsRequest'][0];
		//inside Envelope>Body>lcm:SubmitObjectsRequest
		var bodyRegistryObjectList = lcmSubmitObjectsRequest['rim:RegistryObjectList'][0];
		//inside Envelope>Body>lcm:SubmitObjectsRequest>rim:RegistryObjectList
		var bodyExtrinsicObject = bodyRegistryObjectList['rim:ExtrinsicObject'][0];
		var bodyRegistryPackage = bodyRegistryObjectList['rim:RegistryPackage'][0]; // variable not yet implemented
		var bodyClassification = bodyRegistryObjectList['rim:Classification'][0]; // variable not yet implemented
		var bodyAssociation = bodyRegistryObjectList['rim:Association'][0]; // variable not yet implemented
	...
	if (allMatchedResult.length > 0) {
		...
		if (sK[sK.length - 1][1] == 'LeafClass') {
			...
			logginContract('documentQueryFound', fromIP, qTypeForLog, requestedJSON);
		}
		else {
			var matchEntryUUID = [];
		...
		responseAsFailed();
		logginContract('documentQueryNotFound', fromIP, qTypeForLog, requestedJSON);
	}
}

//send next query result chunk for document query, all variable storedglobally
var sendNextQueryResultChunk = function () {
	if (clientIP[netSocket.remoteAddress].i < clientIP[netSocket.remoteAddress].xmlChunks.length) {
		netSocket.write(clientIP[netSocket.remoteAddress].xmlChunks[clientIP[netSocket.remoteAddress].i], function () {
			clientIP[netSocket.remoteAddress].i++;
		});
	}
};

//ProcessData interprete any xmlMessages came through Netsocket=====================================================
function processData(dataIn, fromIP) {
	console.log('XML:\n' + dataIn);
	if (dataIn.includes('Retrieval Log')) {
		logginContract('documentRetrieve', fromIP, null, null, null, dataIn);
	}
	else {
		parseString(dataIn, function (err, result) {
			...
			*/
			const firstResultKey = Object.keys(result)[0]; // Store the first key of the result object for reuse
			if (firstResultKey == 'query:AdhocQueryRequest') {
				console.log('Query requested...');
				documentQuery(result, fromIP);
			} else if (firstResultKey == 'soapenv:Envelope' && result['soapenv:Envelope']['soapenv:Header'][0]['wsa:Action'][0]['_'] == 'urn:ihe:iti:2007:RegisterDocumentSet-b') {
				console.log('RegisterDocumentSet-b...');
				registerDocumentSetb(result, fromIP);
			}
		});
	}
}

//Log smartcontract functions section================================================================================

var instanceComponent = {};

async function logginContract(event, fromIPAddr, initQueryType, initSearchKeyword, documentID, retrieveInfo) {
	console.log("LogginContract Begun.................");
	if (initQueryType && initSearchKeyword) {
		instanceComponent[fromIPAddr] = {
			queryType: initQueryType,
			searchKeyword: initSearchKeyword
		};
	}
	var logMsgNotEmpty = 'Yes';
	var logMessage;
	var timeStamp = new Date().toString();
	var searchedKeywords = '';

	if (event == 'documentQueryFound' || event == 'documentQueryNotFound') {
		searchedKeywords = util.inspect(instanceComponent[fromIPAddr].searchKeyword, { depth: null, colors: true });
	}

	//Log content format defined here
	// 3 types of log is currently in Document Registry Actor
	//log when published document register transaction to blockchain
	if (event == 'registerSuccess') {
		logMessage = "Registering Log | " + "from Document Repository: " + fromIPAddr + " | " + "to Document Registry: " + thisMachineIP + " | " + "Document ID: " + documentID + "|" + timeStamp;
	}
	//log when finished search and start preparing response transaction
	//found result
	else if (event == 'documentQueryFound') {
		logMessage = "Searching Log | " + "from Document Consumer: " + fromIPAddr + " | " + "to Document Registry: " + thisMachineIP + " | " + "Result_Found" + " | " + instanceComponent[fromIPAddr].queryType + " | " + searchedKeywords + " | " + timeStamp;
	}
	//not found result (is this better to use hash as reference to the query?)
	else if (event == 'documentQueryNotFound') {
		logMessage = "Searching Log | " + "from Document Consumer: " + fromIPAddr + " | " + "to Document Registry: " + thisMachineIP + " | " + "Result_Not_Found" + " | " + instanceComponent[fromIPAddr].queryType + " | " + searchedKeywords + " | " + timeStamp;
	}
	//include document retrieve log here to avoid confusion, local repository need registry to publish log for them
	else if (event == 'documentRetrieve') {
		logMessage = retrieveInfo;
	}
	//else is just for avoiding error
	else {
		logMsgNotEmpty = null;
	}
	...
	if (logMsgNotEmpty) {
		myContract.methods.storeLog(logString).send({
			from: deployerAccount
		})
			.then(function (receipt) {
				console.log(receipt);
...
//Main - Net socket wait for any messages===================================================================================
if (experimentMark) {
	...
	netSocket.on('data', function (data) {
		...
		if (clientIP[netSocket.remoteAddress].receiveCounter == 0) {
			...
			if (clientIP[netSocket.remoteAddress].receiveCounter == clientIP[netSocket.remoteAddress].messageChunks) {
				...
				processData(originalMessage, netSocket.remoteAddress); //converting XML to JSON based on Module "xml-js"
